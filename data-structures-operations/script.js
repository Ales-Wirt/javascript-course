...
for (const flight of flights.split('+')) {
  ...
  console.log(
    `${type.startsWith('_Delayed') ? '🔴' : ''} ${type
      .replaceAll('_', ' ')
      .padStart(20)} from ${getCode(from).toUpperCase()} to ${getCode(
      to
...
const game = {
  ...
  printScorePlayers() {
    for (const [i, e] of this.scored.entries()) {
      console.log(`Goal ${i + 1}: ${e}`);
    }
  },
  printOdds() {
    for (const [team, odd] of Object.entries(this.odds)) {
      const str = team === 'x' ? 'draw' : `visctory ${this[team]}`;
...
game.printScorePlayers();
game.computeAverageOdds();
game.printOdds();
